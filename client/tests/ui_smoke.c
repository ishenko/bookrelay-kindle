...
static gboolean expect_search_focus_after_keyboard(GtkWidget *widget, GdkEventFocus *event, gpointer userdata) {
    App *app = userdata;
    VirtualKeyboard *keyboard = g_object_get_data(G_OBJECT(app->keyboard), "bookrelay-keyboard-state");
    if (!first_search_focus_seen) {
        if (!keyboard->native_open)
            g_error("inline search focused before opening the Kindle keyboard");
...
int main(int argc, char **argv) {
    ...
    if (g_getenv("BOOKRELAY_TEST_NATIVE")) {
        ...
        g_signal_connect_after(app.query, "focus-in-event", G_CALLBACK(expect_search_focus_after_keyboard), &app);
